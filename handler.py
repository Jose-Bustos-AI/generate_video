...
def handler(job):
    ...
    # -----------------------------
    # FIX: provide safe defaults + accept both "prompt" and "positive_prompt"
    # -----------------------------
    length = _coerce_int(job_input.get("length"), 81)
    steps = _coerce_int(job_input.get("steps"), 10)

    # seed: accept seed or random if missing / -1
    seed = _coerce_int(job_input.get("seed"), -1)
    if seed == -1:
        seed = int.from_bytes(os.urandom(8), "big") % 2_000_000_000

    # cfg: accept cfg or default
    cfg = _coerce_float(job_input.get("cfg"), 7.0)

    # width/height: defaults + nearest multiple of 16
    original_width = job_input.get("width", 832)
    original_height = job_input.get("height", 480)
    adjusted_width = to_nearest_multiple_of_16(original_width)
    adjusted_height = to_nearest_multiple_of_16(original_height)

    # positive prompt: accept prompt or positive_prompt
    positive = job_input.get("prompt")
    if not positive:
        positive = job_input.get("positive_prompt", "")
    if not positive:
        positive = "a realistic portrait photo, cinematic lighting, high detail"
    ...
    # -----------------------------
    # Apply to workflow (unchanged node ids)
    # -----------------------------
    ...
    prompt["498"]["inputs"]["context_overlap"] = _coerce_int(job_input.get("context_overlap"), 48)
    ...
    if "834" in prompt:
        ...
        if "829" in prompt and "inputs" in prompt["829"]:
    ...
    if lora_count > 0:
        ...
        for i, lora_pair in enumerate(lora_pairs):
            ...
            lora_high_weight = _coerce_float(lora_pair.get("high_weight"), 1.0)
            lora_low_weight = _coerce_float(lora_pair.get("low_weight"), 1.0)
    ...
    for node_id in videos:
        if videos[node_id]:
            return {"video": videos[node_id][0], "seed": seed, "cfg": cfg, "steps": steps, "length": length}
